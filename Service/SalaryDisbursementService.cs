...
public class SalaryDisbursementService : ISalaryDisbursementService
{
    ...
    public async Task<ApiResponseDto<SalaryDisbursementResponseDto>> CreateSalaryDisbursementAsync(CreateSalaryDisbursementRequestDto request)
    {
        try
        {
            ...
            var requestDate = new DateTime(request.SalaryYear, request.SalaryMonth, 1);

            if (requestDate < new DateTime(currentDate.Year, currentDate.Month, 1))
            {
                return ApiResponseDto<SalaryDisbursementResponseDto>
                    .ErrorResponse("Cannot create salary disbursement for past months");
            }
            ...
            foreach (var employee in employees)
            {
                ...
                //await _disbursementRepository.SalaryDisbursementDetails.AddAsync(detail);
    ...
    public async Task<ApiResponseDto<bool>> ProcessDisbursementAsync(int disbursementId)
    {
        try
        {
            var disbursement = await _disbursementRepository
                .GetDisbursementWithDetailsAsync(disbursementId);
            if (disbursement == null)
