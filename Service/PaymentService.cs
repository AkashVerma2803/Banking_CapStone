    ...
    public class PaymentService : IPaymentService
    {
        ...
        public async Task<ApiResponseDto<PaymentResponseDto>> ApprovePaymentAsync(ApprovePaymentRequestDto request)
        {
            try
            {
                ...
                var client = await _clientRepository.GetClientWithAccountsAsync(payment.ClientId);
                var account = client.Accounts.FirstOrDefault(a => a.AccountStatusId == 1);

                if (account.Balance < payment.Amount)
                {
                    return ApiResponseDto<PaymentResponseDto>
                        .ErrorResponse("Insufficient balance in client account");
                }
